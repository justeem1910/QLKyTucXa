...
public class LoaiPhongRepository {
    ...
    public LoaiPhong findById(Integer id) {
        return jdbcTemplate.queryForObject("SELECT * FROM loai_phong WHERE id=?",
                new Object[]{id},
                (rs, rowNum) -> {
                    LoaiPhong lp = new LoaiPhong();
                    lp.setId(rs.getInt("id"));
    ...
    public int update(LoaiPhong lp) {
        return jdbcTemplate.update(
                "UPDATE loai_phong SET ten_loai=? WHERE id=?",
                lp.getTenLoai(), lp.getId()
        );
    }

    public int delete(Integer id) {
        return jdbcTemplate.update("DELETE FROM loai_phong WHERE id=?", id);
    }
}
